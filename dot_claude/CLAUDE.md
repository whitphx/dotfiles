...
**Don't explain how the change works — the reviewer has the diff open.** The "provable by the diff" cut above extends to design rationale, which is the form I keep over-writing: how something is scoped, what a context manager clears on exit, what happens on the path that isn't taken, why a helper is shaped the way it is. A reviewer confirms all of that by reading the code, so it belongs in a code comment or the commit body if anywhere. What earns space in a PR body is what the diff cannot show: the problem that motivated the change, measurements, and why an obvious alternative was rejected. Draft the description, then delete every sentence a reviewer could verify by reading the diff. State each fact once — "bit-identical", "`torch.equal` true", and "max absolute difference 0" are one fact wearing three costumes.
